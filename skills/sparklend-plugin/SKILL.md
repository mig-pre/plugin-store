...
# sparklend — SparkLend Lending & Borrowing
...
```bash
# Verify sparklend is installed
sparklend --version   # Expected: 0.1.0
...
| onchainos wallet contract-call | Transaction broadcast | On-chain — authoritative |
...
3. **Browse market rates** — run `sparklend reserves` to show available assets and current APYs. Ask what they want to supply or borrow.
4. **Preview first supply** — run `sparklend supply --asset <chosen_asset> --amount <amount>` without `--confirm` so they see the preview before any on-chain action.
5. **Execute supply** — once they confirm, re-run with `--confirm`.
6. **Monitor position** — after supply, run `sparklend health-factor` to show the account summary.
7. **Guide borrow if needed** — if the user wants to borrow, explain the health factor risk and preview with `sparklend borrow --asset <asset> --amount <amount>`.
...
sparklend supply --asset DAI --amount 1000
...
```bash
# Preview borrow:
sparklend borrow --asset USDC --amount 500

# Execute borrow:
sparklend --confirm borrow --asset USDC --amount 500
```
...
sparklend --confirm repay --asset USDC --amount 100

# Repay full outstanding balance:
sparklend --confirm repay --asset USDC --all
```
...
sparklend --confirm withdraw --asset DAI --amount 500

# Withdraw full supplied balance:
sparklend --confirm withdraw --asset DAI --all
```
...
# Execute:
sparklend --confirm supply --asset DAI --amount 1000
sparklend --confirm supply --asset wstETH --amount 1.0
sparklend --confirm supply --asset WETH --amount 0.5   # auto-wraps ETH if WETH balance is insufficient
```
...
```bash
# Preview:
sparklend withdraw --asset DAI --amount 500
sparklend withdraw --asset DAI --all   # withdraw full balance

# Execute:
sparklend --confirm withdraw --asset DAI --amount 500
sparklend --confirm withdraw --asset wstETH --all
```
...
# Execute:
sparklend --confirm borrow --asset DAI --amount 1000
sparklend --confirm borrow --asset WETH --amount 0.1
```
...
```bash
# Preview:
sparklend repay --asset USDC --amount 200

# Execute:
sparklend --confirm repay --asset DAI --amount 500
sparklend --confirm repay --asset USDC --all   # repay full balance including accrued interest
```
...
```bash
sparklend positions
sparklend positions --from 0xYourAddress
```
...
```bash
sparklend reserves
sparklend reserves --asset DAI
sparklend reserves --asset 0x6B175474E89094C44Da98b954EedeAC495271d0F
```
