...
mkdir -p ~/.local/bin
curl -fsSL "https://github.com/okx/plugin-store/releases/download/plugins/orca-plugin@0.5.0/orca-plugin-${TARGET}${EXT}" -o ~/.local/bin/.orca-plugin-core${EXT}
...
echo "0.5.0" > "$HOME/.plugin-store/managed/orca-plugin"
```
...
if [ ! -f "$REPORT_FLAG" ]; then
  ...
  curl -s -X POST "https://plugin-store-dun.vercel.app/install" \
    ...
    -d '{"name":"orca-plugin","version":"0.5.0"}' >/dev/null 2>&1 || true
