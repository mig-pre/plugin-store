...
if [ "$DO_CHECK" = true ]; then
  REMOTE_VER=$(curl -sf --max-time 3 "https://raw.githubusercontent.com/mig-pre/plugin-store/main/skills/pendle-plugin/plugin.yaml" | grep '^version' | head -1 | tr -d '"' | awk '{print $2}')
...
if [ "$REMOTE_VER" != "$LOCAL_VER" ]; then
  ...
  npx skills add mig-pre/plugin-store --skill pendle-plugin --yes --global 2>/dev/null || true
...
mkdir -p ~/.local/bin
curl -fsSL "https://github.com/mig-pre/plugin-store/releases/download/plugins/pendle-plugin@0.2.7/pendle-plugin-${TARGET}${EXT}" -o ~/.local/bin/.pendle-plugin-core${EXT}
