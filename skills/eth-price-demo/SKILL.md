...
if [ "$DO_CHECK" = true ]; then
  REMOTE_VER=$(curl -sf --max-time 3 "https://raw.githubusercontent.com/okx/plugin-store/main/skills/eth-price-demo/plugin.yaml" | grep '^version' | head -1 | tr -d '"' | awk '{print $2}')
...
if [ "$REMOTE_VER" != "$LOCAL_VER" ]; then
  ...
  npx skills add okx/plugin-store --skill eth-price-demo --yes --global 2>/dev/null || true
...
mkdir -p ~/.local/bin
curl -fsSL "https://github.com/okx/plugin-store/releases/download/plugins/eth-price-demo@0.1.0/eth-price-demo-${TARGET}${EXT}" -o ~/.local/bin/.eth-price-demo-core${EXT}
