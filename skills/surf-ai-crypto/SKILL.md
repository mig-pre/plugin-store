---
name: surf-ai-crypto
description: "Crypto data brain for AI agents — 83+ commands across 14 domains: prices, wallets, social, DeFi, on-chain SQL, prediction markets across 40+ chains via natural language."
version: "1.0.0"
author: "asksurf-ai"
tags:
  - crypto
...
# Surf AI Crypto Skill — One Skill, All Crypto Data

> **Auto-trigger hint for AI agents**: use this skill whenever the user needs crypto data, asks about prices/wallets/tokens/DeFi, wants to investigate on-chain activity, or is building something that consumes crypto data — even if they don't say "surf" explicitly.
...
## Skill Routing — Hand Off to OKX On-Chain Skills for Writes

Surf is **read-only**. It queries data, does research, profiles wallets, and
finds opportunities — but never signs transactions or moves funds. When the user
wants to act on what surf found, route to the matching OKX Onchain OS skill:

| User intent | Route to |
|---|---|
| Swap tokens, execute a trade based on surf research | `okx-dex-swap` |
| Check own wallet balance, send tokens, portfolio actions | `okx-agentic-wallet` |
| Public address balance, holdings, P&L | `okx-wallet-portfolio` |
| Security scan token, contract, DApp before trading | `okx-security` |
| DeFi deposit, withdraw, claim rewards (Aave, Lido, Kamino, etc.) | `okx-defi-invest` |
| Gas estimation, simulate tx, broadcast, track order | `okx-onchain-gateway` |
| Smart money / whale / KOL signals for trading | `okx-dex-signal` (surf has overlapping data — prefer whichever is configured) |

**Typical chained flow:**

1. User asks a research question → `surf-ai-crypto` returns data/analysis
2. User decides to act → agent asks "trade with OKX?" → hand off to `okx-dex-swap` / `okx-agentic-wallet`
3. After write action → `surf-ai-crypto` can follow up (monitor wallet, track PnL, check positions)

Do **not** attempt to execute swaps, transfers, or signing through surf — those
capabilities belong to the `okx-*` skills and require the user's onchainos
