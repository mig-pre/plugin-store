...
pub async fn run(args: RemoveLiquidityArgs, chain_id: u64) -> anyhow::Result<()> {
    ...
    let dec_result = wallet_contract_call(chain_id, nfpm, &decrease_data, true, args.dry_run, Some(&wallet)).await?;
    ...
    let col_result = wallet_contract_call(chain_id, nfpm, &collect_data, true, args.dry_run, Some(&wallet)).await?;
