...
> 完成后，你将拥有一个用户可通过
> `npx skills add mig-pre/plugin-store --skill <name>` 安装的 Plugin。
...
> `npx skills add mig-pre/plugin-store --skill <name>` 安装你完成的 Plugin ——
> 用户端无需安装 CLI。
...
然后从你的 fork 向 `mig-pre/plugin-store` 提交 Pull Request。使用以下标题：
...
6. **用户如何安装我的 Plugin？**
   PR 合并后：`npx skills add mig-pre/plugin-store --skill my-plugin`。用户端无需安装 plugin-store CLI。
...
- 在 GitHub 上提交 [issue](https://github.com/mig-pre/plugin-store/issues)
- 查看 `skills/` 中的现有 Plugin 作为示例
- 提交前在本地运行 lint 命令 —— 它能捕获大多数问题
- 如果你的 PR 检查失败，请查看 [GitHub Actions 日志](https://github.com/mig-pre/plugin-store/actions)
