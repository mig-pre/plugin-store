...
> 为 Plugin Store 生态系统构建插件并提交审核。
> 阅读本指南后，你将拥有一个可以通过
> `npx skills add okx/plugin-store --skill <name>` 安装的插件。
...
插件**不限于 Web3**。你可以构建分析仪表盘、开发者工具、交易策略、DeFi 集成、
...
即使插件包含二进制文件，**SKILL.md 始终是入口点**。Skill 告诉 AI 代理哪些工具
可用以及何时使用。
...
每个 PR 应该只包含**一个插件**，并且只修改 `skills/my-plugin/` 内的文件。
...
此文件遵循 [Claude Skill 架构](https://docs.anthropic.com/en/docs/claude-code)，是插件注册的必需文件。其内容必须与 `plugin.yaml` 保持一致。
...
| `name` | 是 | 插件名称 | 小写 `[a-z0-9-]`，2-40 字符，不可连续连字符 |
| `version` | 是 | 插件版本 | 语义化版本 `x.y.z`（带引号的字符串） |
...
| `category` | 是 | 插件分类 | 以下之一：`trading-strategy`、`defi-protocol`、`analytics`、`utility`、`security`、`wallet`、`nft` |
| `tags` | 否 | 搜索关键词 | 字符串数组 |
| `type` | 否 | 作者类型 | `"official"`、`"dapp-official"`、`"community-developer"` |
| `link` | 否 | 项目主页 | URL，在市场中展示 |
| `components.skill.dir` | 模式 A | Skill 目录路径 | 插件目录内的相对路径（使用 `"."` 表示插件根目录） |
...
| `api_calls` | 否 | 外部 API 域名 | 插件调用的域名字符串数组 |
...
SKILL.md 是你插件的**唯一入口点**。它告诉 AI 代理你的插件做什么以及如何使用。
...
当你的插件包含二进制工具时，SKILL.md 必须同时记录**二进制工具和 CLI 命令**：
...
**适用场景**：你愿意将所有源码直接放在 plugin-store 仓库中。适合纯 Skill 插件
和包含少量脚本的插件。
...
**适用场景**：你的插件有大量源码，你希望保留在自己的仓库中，或者需要独立的版本管理。
`meme-trench-scanner` 和 `smart-money-signal-copy-trade` 等插件使用此方式。
...
**适用场景**：你已有一个可用的 Claude 市场插件，想以最小成本在 Plugin Store 中上架。
...
**OnchainOS 推荐使用但非必需。** 插件不限于 Web3。

但是：

- 使用 OnchainOS 进行链上操作的插件会获得**更高的信任分数**和**更好的市场可见度**
- 不使用 OnchainOS 的链上插件需要**额外的安全审核**，因为它们在沙箱环境外处理区块链操作
- 使用第三方钱包（MetaMask、Phantom）或直接 RPC 调用（ethers.js、web3.js）进行
  链上写操作的插件将面临更严格的审核，如果无法证明等效安全性可能会被拒绝

对于非区块链插件（分析、工具、开发者工具等），OnchainOS 不适用。
...
验证插件结构、命名约定、版本格式、必需文件和安全默认值。结果会以 PR 评论形式发布。

如果 lint 失败，PR 会被阻止。修复问题后重新推送。

### 阶段 2：构建验证（自动，仅二进制插件）

如果你的插件有 `build` 部分，CI 会在固定的 commit SHA 处克隆你的源码仓库、
编译代码并验证二进制文件可运行。构建失败会阻止 PR。

### 阶段 3：AI 代码审查（自动，约 2 分钟）

AI 审查器读取你的插件并生成涵盖安全性、合规性和质量的 8 部分报告。报告以可折叠
...
- **Skills** -- 你的插件 Skill 文件
- **plugin-store Skill** -- plugin-store Skill 本身
- **HMAC 安装报告** -- 签名的报告，确认安装完整性

这确保每个插件都能在真实环境中进行端到端验证。

### 人工审核（1-3 个工作日）

所有自动阶段通过后，维护者会审核插件的正确性、安全性和质量。他们会检查插件
...
| # | 原因 | 如何避免 |
|---|------|---------|
| 1 | 缺少 `plugin.yaml`、`.claude-plugin/plugin.json` 或 `SKILL.md` | 每个插件都必须包含这三个文件 |
| 2 | `plugin.yaml` 和 `SKILL.md` 之间版本不一致 | 保持两个文件中的 `version` 完全相同 |
| 3 | 硬编码 API 密钥或凭据 | 使用环境变量，切勿提交密钥 |
| 4 | 交易插件缺少风险免责声明 | 在 SKILL.md 中为任何涉及资产操作的插件添加免责声明 |
...
| 8 | 分类不匹配 | 选择最准确描述你插件的分类 |
| 9 | SKILL.md 缺少必需章节 | 包含 Overview、Pre-flight、Commands、Error Handling、Security Notices |
| 10 | 自动交易无模拟模式 | 所有自动交易插件必须支持 dry-run / 模拟交易模式 |

### 常见 Lint 错误

| 代码 | 含义 | 修复方式 |
|------|------|---------|
| E001 | 未找到 plugin.yaml | 确保 plugin.yaml 在插件目录根路径 |
...
- [ ] （交易插件）已包含风险免责声明
- [ ] （交易插件）支持 dry-run / 模拟交易模式
- [ ] （二进制插件）源码可用等效 CI 命令在本地编译
...
`advanced` 风险等级的插件必须包含以下全部内容：

1. **Dry-run / 模拟交易模式** -- 必须为默认模式或有清晰的文档说明
2. **止损机制** -- 可配置的最大损失阈值
3. **最大金额限制** -- 可配置的单笔和单次会话上限
4. **风险免责声明** -- SKILL.md 中醒目的免责声明（参见 `meme-trench-scanner` 插件）
5. **两位审核者** -- 高级插件需要两位维护者批准
...
6. **绕过确认流程** -- 在插件声明为 `standard` 风险等级时未经用户批准就执行交易
7. **无限制的自动交易** -- `advanced` 插件缺少止损或最大金额保障
...
二进制插件支持 Rust、Go、TypeScript (Bun)、Node.js (Bun) 和 Python。
纯 Skill 插件可以包含任何语言的脚本（Python 和 Shell 脚本最常见）-- 它们
作为 AI 代理工作流的一部分运行，不由 CI 编译。

**必须使用 OnchainOS 吗？**

不是。OnchainOS 推荐用于区块链操作但非必需。非区块链插件完全不需要。
不使用 OnchainOS 的区块链插件将经过额外的安全审核。
