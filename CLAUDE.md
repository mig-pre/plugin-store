...
`plugin-store self-update` downloads the latest release binary from `mig-pre/plugin-store` on GitHub, resolves the platform target via `utils::platform::current_target()`, and does an atomic rename-based replacement of the running executable with rollback on failure.
...
npx skills add mig-pre/plugin-store --skill <name>  # Install via npx (recommended)
